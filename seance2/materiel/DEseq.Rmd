...
- Dans un premier temps, créez un nouveau R markdown (ou réutilisez celui de la séance précédente) et chargez les libraries dont vous aurez besoin.  
...
# Détection de gènes différentiellement exprimés entre WT vs arp6-1 en faible concentration (0.5 mM) de nitrate
...
- Le dossier dans lequel il y a les données d'expression, avec un fichier par échantillon  
...
- Maintenant, nous suppressimons les gènes non exprimés ou faiblement exprimés. La présence de ces gènes peut poser problème lors de la détection de gènes différentiellement exprimés. Pour cela nous utilisons les commander suivantes:
```{r}
...
- Maintenant nous pouvons ajouter un seuil pour la p-value ajustée à 5%. Les gènes avec une p-value ajustée sous cette valeur sont différentiellement exprimés. 
```{r}
...
> Est-ce que la détection de gènes différentiellement exprimé est la même 
...
- Regarder le résultat avec la fonction suivante. 
```{r}
...
- Finalement, exportez le résultat afin de les conserver sans avoir à refaire toute l'analyse:
```{r}
