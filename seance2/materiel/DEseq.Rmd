...
- Maintenant nous pouvons ajouter un seuil pour la p-value ajustée à 1%. Les gènes avec une p-value ajustée inférieure à cette valeur sont différentiellement exprimés. 
```{r}
res_0.5mM <- results(dds_0.5mM, alpha=0.01)
res_0.5mM
```
...
```{r}
sum(res_0.5mM$padj < 0.01, na.rm=TRUE)
```
...
plus (0.5%) ou moins (5%) stringeant?
...
res_10mM <- results(dds_10mM, alpha=0.01)
...
res_WT <- results(dds_WT, alpha=0.01)
...
res_arp6 <- results(dds_arp6, alpha=0.01)
