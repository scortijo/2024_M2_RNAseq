...
Afin de regrouper des gènes qui ont des expression similaires entre eux dans les différentes conditions, nous allons effectuer un clustering hierarchisé et représenter les résultats sous la forme d'une heatmap.  
...
```{r}
# Calcul des correlations de pearson entre tous les gènes
...
# Creation du dendrograme à partie des distances euclidiennes mesurées avec la fonction `as.dist`
...
# Définition des clusters (ici nous cherchons 6 clusters)
...
> Est-ce que vosu pensez que 6 clusters soit le nombre adapté à nos données?   
...
> Utilisez `separate` afin de séparer la colonne contenant le nom ces échantillons en deux colonnes: une avec le génotype, l'autre avec la concentration en KNO3.   
...
> A partir des résultats obtenus aujourd'hui, sélectionnez un (ou deux maximum) clusters que vous étudierez plus en détail à la prochaine séance. 
